...
config = {
    'apiKey': "AIzaSyBCOosqT__R9LubU8WUFnaX8I8QMXtI_Tg",
    'authDomain': "ptms-c2a6b.firebaseapp.com",
    'databaseURL': "https://ptms-c2a6b.firebaseio.com",
    'projectId': "ptms-c2a6b",
    'storageBucket': "ptms-c2a6b.appspot.com",
    'messagingSenderId': "1071901687607"
  }
...
def fare(request):
	...
	if request.session.has_key('username'):
		...
		print(lat)
		#
		countbycamera1 = str(db.child("passengerCount").child("bus_101").child("countin").get().val())
		print(countbycamera1)
		countbycamera2 = str(db.child("passengerCount").child("bus_101").child("countout").get().val())
		print(countbycamera1)
		#
		...
		count1=0
		count2=0
		lastvisited=db.child("Location").child(bid).child("lastVisitedCity").get().val()
		route=db.child("Bus_Route_time").child(bid).child("Route_ID").get().val()
		paths=str(db.child("Route").child(route).child("Path").get().val())
		path_list=paths.split('#')
		path_names=[]
		for i in path_list:
			print(i)
			path_names.append(str(db.child("City").child(i).child("City_Name").get().val()))
			if str(db.child("City").child(i).child("City_Name").get().val())==str(lastvisited):
				break

		for j in matching:
			for k in db.child("Ticket_Log").child(bid).child(j).shallow().get().val():
				if str(db.child("Ticket_Log").child(bid).child(j).child(k).child("d_Date").get().val())==date_today:
					count1=int(count1)+int(db.child("Ticket_Log").child(bid).child(j).child(k).child("e_No_Of_Ticket").get().val())
					print("count1"+str(count1))
					for m in path_names:
						if str(db.child("Ticket_Log").child(bid).child(j).child(k).child("c_Destination").get().val())==str(m):
							count2=int(count2)+int(db.child("Ticket_Log").child(bid).child(j).child(k).child("e_No_Of_Ticket").get().val())
							print("count2"+str(count2))
		totalseats=db.child("BusDetails").child(bid).child("Total_Seats").get().val()
		remaining=int(totalseats)-(int(count1)-int(count2))
		token='pk.eyJ1IjoiZGF2ZW1haGFyc2hpIiwiYSI6ImNqdHllZnlmazAwM2M0NHJ4cXR4ZzRleWMifQ.mtH0jFqFrqQDg7boXC9lWQ'					
				...
				for k in ticket_status:
					if str(db.child("Fare_Collected_Status").child(k).child("C_ID").get().val())==i and str(db.child("Fare_Collected_Status").child(k).child("Bus_ID").get().val())==bid and str(db.child("Fare_Collected_Status").child(k).child("Date").get().val())==date_today:
						if (str(db.child("Fare_Collected_Status").child(k).child("Is_Collected").get().val())=="0"):
							...
							print(new_list)
			return render(request,'fare.html',{'data':new_list,'lat':lat,'longi':lon,'seats':remaining,'countbycamera1':countbycamera1,'countbycamera2':countbycamera2, 'token':token})
...
def UpdateFareStatus(request):
	...
	if request.session.has_key('username'):
		...
		return HttpResponseRedirect('/fare/')
	return HttpResponseRedirect('/login')